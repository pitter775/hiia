$(function () {
    ...
    function atualizarCard(response) {
        ...
        // Atualiza a imagem do avatar diretamente em Base64
        if (response.imagem) {
            $('#imagemAvatar').attr('src', response.imagem);
        } else {
            $('#imagemAvatar').attr('src', '/assets/img/semfoto.jpg');
        }
    }
    ...
    $('#confirmarCorte').on('click', function () {
        ...
        $('#imagemAvatar').attr('src', base64Image);
        $('#imagemFinal').val(base64Image);
        $('#modalCorteImagem').modal('hide');
    });

    // Função para salvar a imagem
    $('#salvarImagem').on('click', function () {
        const base64Image = $('#imagemFinal').val();
        if (!base64Image) {
        ...
        $.ajax({
            ...
            url: '/images/upload',
            data: {
                image: base64Image.split(',')[1], // Remove o prefixo data:image/jpeg;base64,
                model_type: 'Modelo', // Nome do modelo
                model_id: $('#model_id').val(), // ID do modelo
                _token: $('meta[name="csrf-token"]').attr('content')
            },
            success: function (response) {
                if (response.success) {
                    $('#imagemAvatar').attr('src', response.image); // Atualiza o avatar com a imagem em Base64
                    toastr.success('Imagem salva com sucesso!');
                } else {
                    toastr.error('Erro ao salvar a imagem.');
                }
            },
            error: function () {
                toastr.error('Erro ao salvar a imagem.');
